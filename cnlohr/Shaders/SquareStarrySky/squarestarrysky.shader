// Generic sky shader, but with stars at night and better ambiance for day/night transitions.
// Unity built-in shader source. Copyright (c) 2016 Unity Technologies. MIT license (see license.txt)
// Star algorithm based on nimits https://www.shadertoy.com/view/XtGGRt aurora.
...
Properties {
	...
	[ToggleUI] _SquareTheSun( "Square the Sun", Int ) = 0
...
//              for(int i=0; i<int(kSamples); i++)
                {
                    ...
                    frontColor += attenuate * (depth * scaledLength) * (1.-nightness*.9);
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        half4 frag (v2f IN, float4 Pos : SV_Position ) : SV_Target
        {
        ...
        // if y < 0 [eyeRay.y > 0] - sky
			...
			#endif
		#else
			fixed3 SunAttenuation = 0;
		#endif

        #if SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
			fixed3 sunColor = IN.sunColor;
			if(ray.y < 0.0)
            {
